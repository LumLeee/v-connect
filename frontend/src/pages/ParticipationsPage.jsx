...
export default function ParticipationsPage({ managed = false }) {
  const { id } = useParams()
  const [params, setParams] = useSearchParams()
  const page = Math.max(1, Number(params.get('page')) || 1)
  const { data, loading, error, retry } = useApi(`${managed ? `/organizer/activities/${id}/applicants/` : '/participations/'}?page=${page}&page_size=12`)
  ...
  return <section className="page-width section interior activity-page">
    ...
    <h1>{managed ? 'Danh sách đăng ký' : 'Đăng ký của tôi'}</h1>
    <p>Đăng ký, hủy và xét duyệt chỉ thực hiện trước giờ bắt đầu hoạt động.</p>
    ...
    {data && <>
      <p>{data.count} đơn đăng ký.</p>
      {!data.count && <p className="activity-empty">Chưa có đơn đăng ký.</p>}
